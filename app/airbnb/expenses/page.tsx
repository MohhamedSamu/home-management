...
export default function AirbnbExpensesPage() {
  ...
  const categories = ['general', 'cleaning', 'maintenance', 'supplies', 'utilities', 'amenities', 'other']
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    try {
      const { error } = await supabase.from('airbnb_expenses').insert({
        user_id: userId,
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const { error } = await supabase.from('airbnb_expenses').delete().eq('id', id)
      if (error) throw error
      fetchExpenses()
    } catch (error) {
    ...
    <main className="min-h-screen p-8">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="text-4xl font-bold">Airbnb Expenses</h1>
        </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border rounded-lg"
                  placeholder="e.g., Cleaning service, Maintenance, etc."
                />
              </div>
              <div>
