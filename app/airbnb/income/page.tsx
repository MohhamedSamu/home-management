...
export default function AirbnbIncomePage() {
  ...
  const roomOptions = ['Parinak', 'Parinak 1', 'Parinak 2']
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    try {
      const { error } = await supabase.from('airbnb_income').insert({
        user_id: userId,
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const { error } = await supabase.from('airbnb_income').delete().eq('id', id)
      if (error) throw error
      fetchIncomes()
    } catch (error) {
    ...
    <main className="min-h-screen p-8">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="text-4xl font-bold">Airbnb Income</h1>
        </div>
