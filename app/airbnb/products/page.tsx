...
export default function AirbnbProductsPage() {
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const matchesSupplier = !currentSupplier || 
      (product.supplier && product.supplier.toLowerCase().includes(currentSupplier.toLowerCase()))
  ...
  const handleAddToCart = (product: Product) => {
    ...
    setNewProductData({
      ...
      brand: product.brand || '',
      supplier: currentSupplier || product.supplier || '',
  ...
  const handleAddNewProductToCart = () => {
    if (!newProductData.name || !newProductData.price || !newProductData.supplier) {
      alert('Please fill in name, price, and supplier')
      return
    }
  ...
  const handleSaveCart = async () => {
    ...
    // Use currentSupplier if provided, otherwise use the first item's supplier or "Multiple suppliers"
    ...
    <main className="min-h-screen p-8">
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="text-4xl font-bold">Airbnb Products & Shopping</h1>
        </div>

        <div className="mb-4">
          <label className="block text-sm font-medium mb-1">Filter by Supplier (Optional)</label>
          <input
            type="text"
            value={currentSupplier}
            onChange={(e) => setCurrentSupplier(e.target.value)}
            placeholder="Search/filter by supplier name..."
                      ...
                      <div className="flex-1">
                        ...
                        {product.last_price && (
                          <p className="text-sm font-medium mt-1">
                            ...
                            {product.supplier && ` at ${product.supplier}`}
                          </p>
                        )}
              ...
              <div className="mt-4 p-4 border rounded-lg">
                ...
                {selectedProduct && selectedProduct.last_price && (
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
                    Last price: ${selectedProduct.last_price.toFixed(2)} at {selectedProduct.supplier || 'unknown'}
                  </p>
                )}
