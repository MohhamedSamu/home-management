...
-- Note: We'll insert these in the application code after RLS is enabled
...
-- Indexes
CREATE INDEX idx_wedding_categories_user_id ON wedding_categories(user_id);
CREATE INDEX idx_wedding_expenses_user_id ON wedding_expenses(user_id);
CREATE INDEX idx_wedding_expenses_category_id ON wedding_expenses(category_id);
CREATE INDEX idx_wedding_expenses_date ON wedding_expenses(date);
CREATE INDEX idx_wedding_budgets_user_id ON wedding_budgets(user_id);
CREATE INDEX idx_wedding_budget_items_budget_id ON wedding_budget_items(budget_id);
CREATE INDEX idx_wedding_budget_items_category_id ON wedding_budget_items(category_id);
CREATE INDEX idx_wedding_quotes_user_id ON wedding_quotes(user_id);
CREATE INDEX idx_wedding_quotes_category_id ON wedding_quotes(category_id);
CREATE INDEX idx_wedding_folders_user_id ON wedding_folders(user_id);
CREATE INDEX idx_wedding_notes_user_id ON wedding_notes(user_id);
CREATE INDEX idx_wedding_notes_folder_id ON wedding_notes(folder_id);
...
CREATE POLICY "Users can view their own wedding budget items" ON wedding_budget_items
  FOR SELECT USING (EXISTS (SELECT 1 FROM wedding_budgets WHERE wedding_budgets.id = budget_id AND wedding_budgets.user_id = auth.uid()));

CREATE POLICY "Users can insert their own wedding budget items" ON wedding_budget_items
  FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM wedding_budgets WHERE wedding_budgets.id = budget_id AND wedding_budgets.user_id = auth.uid()));

CREATE POLICY "Users can update their own wedding budget items" ON wedding_budget_items
  FOR UPDATE USING (EXISTS (SELECT 1 FROM wedding_budgets WHERE wedding_budgets.id = budget_id AND wedding_budgets.user_id = auth.uid()));

CREATE POLICY "Users can delete their own wedding budget items" ON wedding_budget_items
  FOR DELETE USING (EXISTS (SELECT 1 FROM wedding_budgets WHERE wedding_budgets.id = budget_id AND wedding_budgets.user_id = auth.uid()));
